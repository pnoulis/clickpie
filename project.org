...
* Ngrok
** About
Ngrok's is a kind of server that exposes a local port to the web via a public
domain. It is very usefull for development purposes as it allows quickly serving
a local application globally through a public domain without the need for
buying a domain name etc.
** Usage
